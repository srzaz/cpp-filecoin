...
struct MessageVector {
  static auto decode(Json::OptIn j) {
    using namespace Json;
    MessageVector mv;
    EXPECT_EQ(*str(get(j, "class")), "message");
    mv.car = gunzip(*bytes(get(j, "car")));
    auto pre{get(j, "preconditions")}, post{get(j, "postconditions")};
    mv.state_before = *cid(get(get(pre, "state_tree"), "root_cid"));
    mv.state_after = *cid(get(get(post, "state_tree"), "root_cid"));
    mv.messages = *list(get(j, "apply_messages"), [&](auto j) {
      return std::make_pair(
          *Int(get(j, "epoch")),
          fc::codec::cbor::decode<UnsignedMessage>(*bytes(get(j, "bytes")))
              .value());
    });
    mv.receipts = *list(get(post, "receipts"), [](auto j) {
      return MessageReceipt{fc::vm::VMExitCode{*Int(get(j, "exit_code"))},
                            *bytes(get(j, "return")),
                            *Int(get(j, "gas_used"))};
    });
    return mv;
...
TEST_P(TestVectors, Vector) {
  ...
  auto jdoc{fc::codec::json::parse(*fc::common::readFile(path))};
  EXPECT_TRUE(jdoc);
